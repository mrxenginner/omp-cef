...
void ViewTexture::ReleaseResources()
{
    ...
    if (rwTexture_) { RwTextureDestroy(rwTexture_); rwTexture_ = nullptr; rwRaster_ = nullptr; }
}
...
bool ViewTexture::Create(LPDIRECT3DDEVICE9 device, int width, int height)
{
    ...
    if (FAILED(hr)) { LOG_ERROR("[ViewTexture] Failed to create D3D9 texture ({}x{})", width, height); return false; }

    rwRaster_ = RwRasterCreate(width, height, 32, rwRASTERTYPETEXTURE | rwRASTERFORMAT8888);
    if (!rwRaster_) { LOG_ERROR("[ViewTexture] Failed to create RwRaster"); ReleaseResources(); return false; }

    rwTexture_ = RwTextureCreate(rwRaster_);
    if (!rwTexture_) { LOG_ERROR("[ViewTexture] Failed to create RwTexture from RwRaster"); RwRasterDestroy(rwRaster_); rwRaster_ = nullptr; ReleaseResources(); return false; }
...
void ViewTexture::Update(const void* pixels, int width, int height)
{
    if (!texture_ || !rwRaster_ || width != width_ || height != height_) {
        if (width > 0 && height > 0) { LOG_WARN("[ViewTexture] Size mismatch or invalid state, recreating (old={}x{}, new={}x{})", width_, height_, width, height); Create(device_, width, height); }
        if (!texture_ || !rwRaster_) return;
    }
    ...
    if (SUCCEEDED(texture_->LockRect(0, &rect, nullptr, 0)))
    {
        ...
        if (rect.Pitch == width * 4) { memcpy(dst, src, static_cast<size_t>(width) * height * 4); }
        else {
    ...
    if (rasterPixels) {
        ...
        if (rwPitch == srcPitch) { memcpy(rasterPixels, src, static_cast<size_t>(height) * srcPitch); }
        else {
            for (int y = 0; y < height; ++y) {
...
{
    if (!texture_ || !rwRaster_ || !pixels || width <= 0 || height <= 0) return;
