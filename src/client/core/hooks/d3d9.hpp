...
extern volatile IDirect3D9*&        gGameDirect;
extern volatile IDirect3DDevice9*&  gGameDevice;

class IDirect3D9Hook : public IDirect3D9
{
public:
    IDirect3D9Hook() = delete;
    ~IDirect3D9Hook() noexcept { _orig->Release(); }

    IDirect3D9Hook(const IDirect3D9Hook&) = delete;
    IDirect3D9Hook& operator=(const IDirect3D9Hook&) = delete;

    explicit IDirect3D9Hook(IDirect3D9* orig) noexcept : _orig{ orig } { _orig->AddRef(); }
