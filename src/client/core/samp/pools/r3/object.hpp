...
class SampObject_R3 : public ISampObject 
{
public:
    SampObject_R3(sampapi::v037r3::CObject* obj) : m_sampObject(obj) {}
...
private:
    sampapi::v037r3::CObject* m_sampObject;
};

class ObjectPool_R3 : public IObjectPool 
{
public:
    ObjectPool_R3() = default;
    ...
    std::unique_ptr<ISampObject> Get(int objectId) override 
    {
        ...
        sampapi::v037r3::CObject* sampObject = netGame->m_pPools->m_pObject->Get(objectId);
        if (!sampObject)
            return nullptr;

        return std::make_unique<SampObject_R3>(sampObject);
    }
};